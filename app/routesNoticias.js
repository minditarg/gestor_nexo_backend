...
module.exports = function (app, connection, passport) {
  ...
  app.get('/list-transparente-inicial', function (req, res) {
    ...
    try {
      connection.query("select n.*, ut.descripcion as user_type FROM noticias n inner join users u on n.id_user = u.id inner join users_type ut on ut.id = u.id_users_type where n.estado = 1 and n.id_tipo_noticia = 4 and n.fecha_inicio > now() order by n.fecha_inicio limit 10", null, function (err, result) {
        if (err) return res.status(500).send(err);
